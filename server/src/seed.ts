...
async function main() {
  ...
  // Partner contacts
  for (const c of [
    { partner_id: partner1.id, name: 'Dana Torres', title: 'Executive Producer', email: 'dana@meridianfilms.com', phone: '310-555-0101' },
    { partner_id: partner2.id, name: 'Ryan Okafor', title: 'Head of Post', email: 'ryan@atlaspost.com', phone: '212-555-0202' },
    { partner_id: partner3.id, name: 'Priya Nair', title: 'Executive Producer', email: 'priya@wildlight.com', phone: '512-555-0303' },
  ]) {
    await prisma.partnerContact.create({ data: c });
  }
  ...
  for (const s of [
    ...
    await prisma.partnerSpeciality.create({ data: s });
  }
  ...
  for (const pc of [
    ...
    await prisma.partnerCategory.create({ data: pc });
  }
  ...
  for (const b of [
    ...
    await prisma.budgetLineItem.create({ data: b });
  }

  // ── Milestones ────────────────────────────────────────────────────────────
  for (const m of [
    { project_id: project1.id, assignee_id: userProducer1.id, name: 'Treatment review', start_date: addDays(today, -5), end_date: addDays(today, -3), completed: true, sort_order: 1 },
    { project_id: project1.id, assignee_id: userProducer1.id, name: 'Production shoot', start_date: addDays(today, 5), end_date: addDays(today, 6), completed: false, sort_order: 2 },
    { project_id: project1.id, assignee_id: userAssoc.id, name: 'First color pass', start_date: addDays(today, 10), end_date: addDays(today, 14), completed: false, sort_order: 3 },
    { project_id: project1.id, assignee_id: userProducer1.id, name: 'Client delivery', start_date: addDays(today, 28), end_date: addDays(today, 30), completed: false, sort_order: 4 },
    { project_id: project2.id, assignee_id: userProducer2.id, name: 'Concept approval', start_date: addDays(today, 2), end_date: addDays(today, 5), completed: false, sort_order: 1 },
    { project_id: project3.id, assignee_id: userProducer1.id, name: 'Casting', start_date: addDays(today, 3), end_date: addDays(today, 7), completed: false, sort_order: 1 },
  ]) {
  ...
  for (const tm of [
    ...
    await prisma.projectTeamMember.create({ data: tm });
  }
  ...
  for (const pc of [
    ...
    await prisma.projectCategory.create({ data: pc });
  }
  ...
  for (const pp of [
    ...
    await prisma.projectPartner.create({ data: pp });
  }
